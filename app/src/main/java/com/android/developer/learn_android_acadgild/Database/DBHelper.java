...
public class DBHelper {
    ...
    public DBHelper(Context c) {
        context = c;
        dbHelper = new Table(context, Constants.DATABASE_NAME, null, Constants.DATABASE_VERSION);
    }
    ...
    public ArrayList<Data> getAllDatas() {
        ...
        if (cursor.moveToFirst()) {
            do {
                data = new Data();
                data.setId((cursor.getString(0)).toString());
    ...
    public void dropTable(String tableName) {
        String drop = "DROP TABLE IF EXISTS " + Constants.TABLE;
        db.execSQL(drop);
    }
}
